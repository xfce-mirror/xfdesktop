...
static GtkWidget *
menu_item_from_xfw_window(XfwWindow *xfw_window, gint icon_width,
        gint icon_height)
{
    ...
    if(wl_show_icons) {
        ...
        icon = xfw_window_get_icon(xfw_window, icon_size);
        if (icon != NULL) {
    ...
    for (GList *gl = groups; gl != NULL; gl = gl->next, ++group_num) {
             ...
             wl = wl->next)
        {
            ...
            for(GList *l = xfw_screen_get_windows_stacked(xfw_screen); l; l = l->next) {
                ...
                mi = menu_item_from_xfw_window(xfw_window, w, h);
                if(!mi)
                    continue;
