...
menu_arrange_icons(GtkWidget *mi, MonitorData *mdata) {
    ...
    {
        xfdesktop_window_icon_manager_sort_icons(XFDESKTOP_ICON_VIEW_MANAGER(mdata->wmanager), GTK_SORT_ASCENDING);
    }
}
...
build_root_context_menu(MonitorData *mdata) {
    ...
    if (monitor != NULL &&
        workspace != NULL &&
        xfdesktop_backdrop_manager_can_cycle_backdrop(xfdesktop_icon_view_manager_get_backdrop_manager(XFDESKTOP_ICON_VIEW_MANAGER(mdata->wmanager)),
                                                      monitor,
                                                      workspace))
    {
