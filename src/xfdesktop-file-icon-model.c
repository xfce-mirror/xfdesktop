...
static gboolean
volume_is_probably_removable(GVolume *volume) {
...
add_volume_icon(XfdesktopFileIconModel *fmodel, GVolume *volume) {
    DBG("entering, %p", volume);
    if (volume_is_probably_removable(volume)) {
        gchar *volume_type = g_volume_get_identifier(volume, G_VOLUME_IDENTIFIER_KIND_CLASS);

        if (g_strcmp0(volume_type, "network") == 0 || g_strcmp0(volume_type, "device") == 0 || volume_type == NULL) {
