...
struct _XfdesktopIconPositionConfig {
    ...
    GHashTable *monitors;  // string id (owner) -> string display name (owner)
...
typedef enum {
    ...
    PARSER_CONFIG_MAP,
    PARSER_CONFIG_LEVEL,
    PARSER_CONFIG_GEOMETRY,
    PARSER_CONFIG_GEOMETRY_MAP,
    PARSER_CONFIG_GEOMETRY_MAP_X,
    PARSER_CONFIG_GEOMETRY_MAP_Y,
    PARSER_CONFIG_GEOMETRY_MAP_WIDTH,
    PARSER_CONFIG_GEOMETRY_MAP_HEIGHT,
...
#define S(val) case val: \
    ...
    switch (state) {
        ...
        S(PARSER_CONFIG_GEOMETRY)
        S(PARSER_CONFIG_GEOMETRY_MAP)
        S(PARSER_CONFIG_GEOMETRY_MAP_X)
        S(PARSER_CONFIG_GEOMETRY_MAP_Y)
        S(PARSER_CONFIG_GEOMETRY_MAP_WIDTH)
        S(PARSER_CONFIG_GEOMETRY_MAP_HEIGHT)
...
save_icons(XfdesktopIconPositionConfigs *configs, GError **error) {
    ...
    if (!yaml_emitter_initialize(&emitter)) {
        g_message("Failed to create YAML emitter: %s", libyaml_strerror(emitter.error));
        return FALSE;
    }
...
G_STMT_END
    ...
    EMIT(emitter, event);
    {
        ...
        EMIT(emitter, event);
        {
            yaml_mapping_start_event_initialize(&event, NULL, NULL, TRUE, YAML_BLOCK_MAPPING_STYLE);
            EMIT(emitter, event);
            {
                EMIT_UQSTR_EVENT(emitter, event, "configs");

                yaml_sequence_start_event_initialize(&event, NULL, NULL, TRUE, YAML_BLOCK_SEQUENCE_STYLE);
                EMIT(emitter, event);
                {
                    for (GList *l = configs->configs; l != NULL; l = l->next) {
                        ...
                        EMIT(emitter, event);
                        {
                            ...
                            EMIT(emitter, event);
                            {
                                ...
                                EMIT_INT_EVENT(emitter, event, config->geometry.x);
                                EMIT_UQSTR_EVENT(emitter, event, "y");
                                EMIT_INT_EVENT(emitter, event, config->geometry.y);
                                EMIT_UQSTR_EVENT(emitter, event, "width");
                                EMIT_INT_EVENT(emitter, event, config->geometry.width);
                                EMIT_UQSTR_EVENT(emitter, event, "height");
                                EMIT_INT_EVENT(emitter, event, config->geometry.height);
                            ...
                            EMIT(emitter, event);
                            {
                                ...
                                while (g_hash_table_iter_next(&iter, (gpointer)&id, (gpointer)&display_name)) {
                                    yaml_mapping_start_event_initialize(&event, NULL, NULL, TRUE, YAML_BLOCK_MAPPING_STYLE);
                                    EMIT(emitter, event);
                                    {
                                        ...
                                        EMIT_QSTR_EVENT(emitter, event, display_name);
...
validate_position_config(XfdesktopIconPositionConfig *config) {
    if (config->level == XFDESKTOP_ICON_POSITION_LEVEL_INVALID) {
        return "Missing level in config";
    } else if (config->geometry.x == G_MININT) {
        return "Missing geometry->x in config";
    } else if (config->geometry.y == G_MININT) {
        return "Missing geometry->y in config";
    } else if (config->geometry.width == G_MININT) {
        return "Missing geometry->width in config";
    } else if (config->geometry.height == G_MININT) {
        return "Missing geometry->height in config";
    } else {
        return NULL;
    }
}
...
gboolean
xfdesktop_icon_position_configs_load(XfdesktopIconPositionConfigs *configs, XfwScreen *screen, GError **error) {
    ...
    if (!yaml_parser_initialize(&parser)) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_INITIALIZED, "Failed to initialize YAML parser");
        return FALSE;
    }
    ...
    gchar *cur_monitor_display_name = NULL;
                ...
                switch (state) {
                    ...
                    case PARSER_CONFIG_MAP:
                        if (scalar_equal(&event, "level")) {
                            ...
                            STATE_TRANSITION(level, state, PARSER_CONFIG_GEOMETRY);
                    ...
                    case PARSER_CONFIG_GEOMETRY_MAP:
                        if (scalar_equal(&event, "x")) {
                            STATE_TRANSITION(level, state, PARSER_CONFIG_GEOMETRY_MAP_X);
                        } else if (scalar_equal(&event, "y")) {
                            STATE_TRANSITION(level, state, PARSER_CONFIG_GEOMETRY_MAP_Y);
                        } else if (scalar_equal(&event, "width")) {
                            STATE_TRANSITION(level, state, PARSER_CONFIG_GEOMETRY_MAP_WIDTH);
                        } else if (scalar_equal(&event, "height")) {
                            STATE_TRANSITION(level, state, PARSER_CONFIG_GEOMETRY_MAP_HEIGHT);
                        } else {
                            gchar *scalar = scalar_to_str(&event);
                            PARSE_ERROR(level, state, configs, event, "Unexpected scalar in geometry '%s'", scalar);
                            g_free(scalar);
                        }
                        break;

                    case PARSER_CONFIG_GEOMETRY_MAP_X: {
                        g_assert(cur_config != NULL);
                        gchar *err_msg = scalar_to_int(&event, "geometry->x", &cur_config->geometry.x);
                        if (err_msg != NULL) {
                            PARSE_ERROR(level, state, configs, event, "%s", err_msg);
                            g_free(err_msg);
                        } else {
                            STATE_TRANSITION(level, state, PARSER_CONFIG_GEOMETRY_MAP);
                        }
                        break;
                    }

                    case PARSER_CONFIG_GEOMETRY_MAP_Y: {
                        g_assert(cur_config != NULL);
                        gchar *err_msg = scalar_to_int(&event, "geometry->y", &cur_config->geometry.y);
                        if (err_msg != NULL) {
                            PARSE_ERROR(level, state, configs, event, "%s", err_msg);
                            g_free(err_msg);
                        } else {
                    ...
                    case PARSER_CONFIG_GEOMETRY_MAP_WIDTH: {
                        g_assert(cur_config != NULL);
                        gchar *err_msg = scalar_to_int(&event, "geometry->width", &cur_config->geometry.width);
                        if (err_msg != NULL) {
                            PARSE_ERROR(level, state, configs, event, "%s", err_msg);
                            g_free(err_msg);
                        } else {
                    ...
                    case PARSER_CONFIG_GEOMETRY_MAP_HEIGHT: {
                        g_assert(cur_config != NULL);
                        gchar *err_msg = scalar_to_int(&event, "geometry->height", &cur_config->geometry.height);
                        if (err_msg != NULL) {
                            PARSE_ERROR(level, state, configs, event, "%s", err_msg);
                            g_free(err_msg);
                        } else {
                    ...
                    case PARSER_CONFIG_MONITOR_DISPLAY_NAME:
                        g_free(cur_monitor_display_name);
                        cur_monitor_display_name = scalar_to_str(&event);
                        if (cur_monitor_display_name == NULL) {
                ...
                switch (state) {
                    ...
                    case PARSER_CONFIGS_ARRAY:
                        if (cur_config != NULL) {
                            ...
                            STATE_TRANSITION(level, state, PARSER_CONFIG_MAP);
                            GdkRectangle dummy = { .x = G_MININT, .y = G_MININT, .width = G_MININT, .height = G_MININT };
                            cur_config = xfdesktop_icon_position_config_new(G_MININT, &dummy);
                            configs->configs = g_list_prepend(configs->configs, cur_config);
                        }
                        break;

                    case PARSER_CONFIG_GEOMETRY:
                        STATE_TRANSITION(level, state, PARSER_CONFIG_GEOMETRY_MAP);
                        break;
            ...
            case YAML_MAPPING_END_EVENT:
                ...
                switch (state) {
                    ...
                    case PARSER_CONFIG_MONITOR_MAP:
                        ...
                        } else if (cur_monitor_display_name == NULL) {
                            PARSE_ERROR(level, state, configs, event, "Monitor is missing display name");
                        } else {
                            g_hash_table_insert(cur_config->monitors, cur_monitor_id, cur_monitor_display_name);

                            for (GList *l = xfw_screen_get_monitors(screen); l != NULL; l = l->next) {
                                XfwMonitor *monitor = XFW_MONITOR(l->data);
                                if (g_strcmp0(cur_monitor_id, xfw_monitor_get_identifier(monitor)) == 0
                                    && !g_hash_table_contains(configs->config_to_monitor, cur_config))
                                {
                                    g_hash_table_insert(configs->config_to_monitor, cur_config, monitor);
                                }
                            }

                            cur_monitor_id = NULL;
                            cur_monitor_display_name = NULL;
                            STATE_TRANSITION(level, state, PARSER_CONFIG_MONITORS_ARRAY);
                        }
                        break;
                                            ...
                                            GList **candidates)
{
    ...
    for (GList *l = configs->configs; l != NULL; l = l->next) {
        ...
        if (!g_hash_table_contains(configs->config_to_monitor, config)) {
            if (g_hash_table_contains(config->monitors, monitor_id)) {
                ...
                best_config = config;
                break;
            } else if (config->geometry.width == geom.width
                       && config->geometry.height == geom.height)
            {
    ...
    if (best_config == NULL) {
        ...
        if (n_candidates == 0) {
            best_config = xfdesktop_icon_position_config_new(level, &geom);
            ...
            for (GList *l = *candidates; l != NULL; l = l->next) {
                XfdesktopIconPositionConfig *config = l->data;
                if (gdk_rectangle_equal(&geom, &config->geometry)) {
                                               ...
                                               XfwMonitor *monitor)
{
    ...
    const gchar *monitor_id = xfw_monitor_get_identifier(monitor);
    if (!g_hash_table_contains(config->monitors, monitor_id)) {
        g_hash_table_insert(config->monitors,
                            g_strdup(monitor_id),
                            g_strdup(xfw_monitor_get_description(monitor)));
    }

    if (g_list_find(configs->configs, config) == NULL) {
        configs->configs = g_list_prepend(configs->configs, config);
    }
...
XfdesktopIconPositionConfig *
xfdesktop_icon_position_config_new(XfdesktopIconPositionLevel level, GdkRectangle *geometry) {
    g_return_val_if_fail(geometry != NULL, NULL);

    XfdesktopIconPositionConfig *config = g_new0(XfdesktopIconPositionConfig, 1);
    config->level = level;
    config->geometry = *geometry;
    config->monitors = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
...
xfdesktop_icon_position_config_get_monitor_display_names(XfdesktopIconPositionConfig *config) {
    ...
    const gchar *display_name;
    while (g_hash_table_iter_next(&iter, NULL, (gpointer)&display_name)) {
        display_names = g_list_prepend(display_names, (gpointer)display_name);
    }
