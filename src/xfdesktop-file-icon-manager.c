...
xfdesktop_file_icon_manager_constructed(GObject *obj)
{
    ...
    fmanager->drag_targets = gtk_target_list_new(drag_targets,
                                                       G_N_ELEMENTS(drag_targets));
    fmanager->drop_targets = gtk_target_list_new(drop_targets,
...
G_GNUC_END_IGNORE_DEPRECATIONS
    ...
    GError *error = NULL;
    if (!xfdesktop_icon_position_configs_load(fmanager->position_configs, screen, &error)) {
...
create_icon_view(XfdesktopFileIconManager *fmanager, XfceDesktop *desktop) {
    ...
    mdata->filter = xfdesktop_file_icon_model_filter_new(channel, monitor, fmanager->model);
    if (fmanager->ready) {
    ...
    if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(fmanager->model), &iter)) {
        do {
            ...
            if (icon != NULL) {
                ...
                if (xfdesktop_icon_position_configs_lookup(fmanager->position_configs, identifier, &icon_monitor, NULL, NULL)) {
                    if (xfdesktop_icon_get_monitor(XFDESKTOP_ICON(icon)) != icon_monitor) {
                        xfdesktop_icon_set_monitor(XFDESKTOP_ICON(icon), icon_monitor);
                        changed = TRUE;
                    }
                } else {
                    XfwScreen *screen = xfdesktop_icon_view_manager_get_screen(XFDESKTOP_ICON_VIEW_MANAGER(fmanager));
                    XfwMonitor *primary = xfw_screen_get_primary_monitor(screen);
                    xfdesktop_icon_set_monitor(XFDESKTOP_ICON(icon), primary);
                    changed = TRUE;
                }
