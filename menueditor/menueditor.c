...
/****************************************************/
void
browse_command_cb (GtkWidget * widget, gpointer data)
{
  MenuEditor *me;
...
void
browse_icon_cb (GtkWidget * widget, gpointer data)
{
  MenuEditor *me;
  ...
  filesel_dialog =
    xfce_file_chooser_new (_("Select icon"), GTK_WINDOW (me->main_window),
                           XFCE_FILE_CHOOSER_ACTION_OPEN,
                           GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
...
delete_main_window_cb (GtkWidget * widget, GdkEvent * event, gpointer data)
{
  ...
  if (me->menu_modified) {
    ...
    response =
      xfce_message_dialog (GTK_WINDOW (me->main_window), "Question",
...
/*****************************/
void
load_menu_in_tree (xmlNodePtr menu, GtkTreeIter * p, gpointer data)
{
  MenuEditor *me;
  GtkTreeIter c;
  ...
  while (menu != NULL) {
    ...
    if (prop_visible && (!xmlStrcmp (prop_visible, (xmlChar *) "false")
                         || !xmlStrcmp (prop_visible, (xmlChar *) "no")))
      hidden = TRUE;
    ...
    if (!xmlStrcmp (menu->name, (xmlChar *) "separator")) {
      ...
      gtk_tree_store_set (me->treestore, &c,
                          ICON_COLUMN, icon,
                          NAME_COLUMN, name, COMMAND_COLUMN, "", HIDDEN_COLUMN, hidden, POINTER_COLUMN, menu, -1);
    }
    /* launcher */
    if (!xmlStrcmp (menu->name, (xmlChar *) "app")) {
      ...
      gtk_tree_store_set (me->treestore, &c,
                          ICON_COLUMN, icon,
                          NAME_COLUMN, name, COMMAND_COLUMN, cmd, HIDDEN_COLUMN, hidden, POINTER_COLUMN, menu, -1);
    }
    ...
    if (!xmlStrcmp (menu->name, (xmlChar *) "include")) {
      ...
      gtk_tree_store_set (me->treestore, &c,
                          ICON_COLUMN, icon,
                          NAME_COLUMN, name, COMMAND_COLUMN, src, HIDDEN_COLUMN, hidden, POINTER_COLUMN, menu, -1);
    }
    ...
    if (!xmlStrcmp (menu->name, (xmlChar *) "title")) {
      ...
      gtk_tree_store_set (me->treestore, &c,
                          ICON_COLUMN, icon,
                          NAME_COLUMN, title, COMMAND_COLUMN, "", HIDDEN_COLUMN, hidden, POINTER_COLUMN, menu, -1);
    }
...
/*****************************/
static void
menu_open_default_cb (GtkWidget * widget, gpointer data)
{
  MenuEditor *me;
  gchar *window_title;
  gchar *home_menu;
  ...
  if (me->xml_menu_file != NULL && me->menu_modified) {
    ...
    response =
      xfce_message_dialog (GTK_WINDOW (me->main_window), _("Question"),
                           GTK_STOCK_DIALOG_QUESTION,
...
menu_saveas_cb (GtkWidget * widget, gpointer data)
{
  ...
  filesel_dialog =
    xfce_file_chooser_new (_("Save as..."), GTK_WINDOW (me->main_window),
...
treeview_button_pressed_cb (GtkTreeView * treeview, GdkEventButton * event, gpointer data)
{
  ...
  if ((event->button == 3) && (event->type == GDK_BUTTON_PRESS)) {
    ...
    if (gtk_tree_view_get_path_at_pos (treeview, event->x, event->y, &path, NULL, NULL, NULL)) {
      ...
      g_signal_connect ((gpointer) edit_menuitem, "activate", G_CALLBACK (popup_edit_cb), me);
      g_signal_connect ((gpointer) add_menuitem, "activate", G_CALLBACK (add_entry_cb), me);
      g_signal_connect ((gpointer) addmenu_menuitem, "activate", G_CALLBACK (add_menu_cb), me);
...
create_main_window (MenuEditor * me)
{
  ...
  gtk_window_set_title (GTK_WINDOW (me->main_window), "Xfce4-MenuEditor");
  g_signal_connect (G_OBJECT (me->main_window), "delete-event", G_CALLBACK (delete_main_window_cb), me);
  ...
  me->toolbar_save =
    gtk_toolbar_append_element (GTK_TOOLBAR (toolbar),
                                GTK_TOOLBAR_CHILD_BUTTON, NULL, "",
                                _("Save current menu"), NULL, tmp_toolbar_icon, GTK_SIGNAL_FUNC (menu_save_cb), me);
  tmp_toolbar_icon = gtk_image_new_from_stock (GTK_STOCK_CLOSE, GTK_ICON_SIZE_LARGE_TOOLBAR);
  me->toolbar_close =
    gtk_toolbar_append_element (GTK_TOOLBAR (toolbar),
                                GTK_TOOLBAR_CHILD_BUTTON, NULL, "",
                                _("Close current menu"), NULL, tmp_toolbar_icon, GTK_SIGNAL_FUNC (close_menu_cb), me);
  ...
  me->toolbar_collapse =
    gtk_toolbar_append_element (GTK_TOOLBAR (toolbar),
                                GTK_TOOLBAR_CHILD_BUTTON, NULL, "",
                                _("Collapse the tree"), NULL, tmp_toolbar_icon, GTK_SIGNAL_FUNC (collapse_tree_cb), me);
  tmp_toolbar_icon = gtk_image_new_from_stock (GTK_STOCK_ZOOM_IN, GTK_ICON_SIZE_LARGE_TOOLBAR);
  me->toolbar_expand =
    gtk_toolbar_append_element (GTK_TOOLBAR (toolbar),
                                GTK_TOOLBAR_CHILD_BUTTON, NULL, "",
                                _("Expand the tree"), NULL, tmp_toolbar_icon, GTK_SIGNAL_FUNC (expand_tree_cb), me);


  /* Tree View inspirated from Gaim */
  me->treestore =
    gtk_tree_store_new (NUM_COLUMNS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_BOOLEAN, G_TYPE_POINTER);
  ...
  command_column =
    gtk_tree_view_column_new_with_attributes (_("Command"), command_cell, "markup", COMMAND_COLUMN, NULL);
  hidden_column = gtk_tree_view_column_new_with_attributes (_("Hidden"), hidden_cell, "active", HIDDEN_COLUMN, NULL);
  pointer_column = gtk_tree_view_column_new_with_attributes (_("Pointer"), pointer_cell, "text", POINTER_COLUMN, NULL);
  ...
  /* For the menu */
  g_signal_connect (G_OBJECT (me->file_menu_new), "activate", G_CALLBACK (new_menu_cb), me);
  g_signal_connect (G_OBJECT (me->file_menu_open), "activate", G_CALLBACK (menu_open_cb), me);
  g_signal_connect (G_OBJECT (me->file_menu_default), "activate", G_CALLBACK (menu_open_default_cb), me);
  g_signal_connect (G_OBJECT (me->file_menu_save), "activate", G_CALLBACK (menu_save_cb), me);
  g_signal_connect (G_OBJECT (me->file_menu_saveas), "activate", G_CALLBACK (menu_saveas_cb), me);
  g_signal_connect (G_OBJECT (me->file_menu_close), "activate", G_CALLBACK (close_menu_cb), me);
  g_signal_connect (G_OBJECT (me->file_menu_exit), "activate", G_CALLBACK (confirm_quit_cb), me);

  g_signal_connect (G_OBJECT (me->edit_menu_add), "activate", G_CALLBACK (add_entry_cb), me);
  g_signal_connect (G_OBJECT (me->edit_menu_add_menu), "activate", G_CALLBACK (add_menu_cb), me);
  g_signal_connect (G_OBJECT (me->edit_menu_del), "activate", G_CALLBACK (delete_entry_cb), me);
  g_signal_connect (G_OBJECT (me->edit_menu_up), "activate", G_CALLBACK (entry_up_cb), me);
  g_signal_connect (G_OBJECT (me->edit_menu_down), "activate", G_CALLBACK (entry_down_cb), me);

  g_signal_connect (G_OBJECT (me->help_menu_about), "activate", G_CALLBACK (about_cb), me);
  ...
  g_signal_connect (G_OBJECT (hidden_cell), "toggled", G_CALLBACK (visible_column_toggled_cb), me);

  /* Set up dnd */
  gtk_tree_view_enable_model_drag_source (GTK_TREE_VIEW (me->treeview), GDK_BUTTON1_MASK, gte, 3, GDK_ACTION_MOVE);
  gtk_tree_view_enable_model_drag_dest (GTK_TREE_VIEW (me->treeview), gte, 3, GDK_ACTION_COPY);
...
/********************/
static void
open_menu_file (gchar * menu_file, MenuEditor * me)
{
  ...
  if (!user_menu) {
    ...
    for (i = 0; all_dirs[i]; i++) {
      if (strstr (all_dirs[i], userhome) != all_dirs[i]) {
        g_snprintf (searchpath, PATH_MAX * 3 + 2, "%s%%F.%%L:%s%%F.%%l:%s%%F", all_dirs[i], all_dirs[i], all_dirs[i]);
        if (xfce_get_path_localized (filename, PATH_MAX, searchpath, "menu.xml", G_FILE_TEST_IS_REGULAR)) {
          g_strfreev (all_dirs);
...
main (int argc, char *argv[])
{
  ...
  if (argc > 1 && (!strcmp (argv[1], "--version") || !strcmp (argv[1], "-V"))) {
    ...
    g_print ("\tbuilt with GTK+-%d.%d.%d and LIBXML2 v%s,\n",
             GTK_MAJOR_VERSION, GTK_MINOR_VERSION, GTK_MICRO_VERSION, LIBXML_DOTTED_VERSION);
