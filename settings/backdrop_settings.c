...
backdrop_create_channel (McsPlugin * mcs_plugin)
{
    ...
    setting =
	mcs_manager_setting_lookup (mcs_plugin->manager, "setbackground",
				    BACKDROP_CHANNEL);
    if (setting)
    {
	 ...
	 */
	backdrop_color.red = (guint16) 0x9000;
	backdrop_color.green = (guint16) 0xA000;
	backdrop_color.blue = (guint16) 0xB000;
    ...
    {
	mcs_manager_set_int (mcs_plugin->manager, "showimage",
			     BACKDROP_CHANNEL, showimage);
    }
...
static void
showimage_toggle (GtkWidget * b, BackdropDialog * bd)
{
...
static void
add_color_button (GtkWidget * vbox, BackdropDialog * bd)
{
    ...
    gtk_box_pack_start (GTK_BOX (hbox), bd->color_only_checkbox, FALSE, TRUE,
			0);
...
static void
set_dnd_dest (BackdropDialog * bd)
{
    /* file entry */
...
static void
add_file_entry (GtkWidget * vbox, GtkSizeGroup * sg, BackdropDialog * bd)
{
...
static void
set_path_cb (const char *path, BackdropDialog * bd)
{
    if (!is_running)
	return;
...
static void
edit_list_cb (GtkWidget * w, BackdropDialog * bd)
{
...
void
new_list_cb (GtkWidget * w, BackdropDialog * bd)
{
    create_list_file (bd->dialog, (ListMgrCb) set_path_cb, (gpointer) bd);
}

static void
add_button_box (GtkWidget * vbox, GtkSizeGroup * sg, BackdropDialog * bd)
{
    ...
    gtk_box_pack_start (GTK_BOX (hbox), bd->edit_list_button, FALSE, FALSE,
			0);
...
add_style_options (GtkWidget * vbox, GtkSizeGroup * sg, BackdropDialog * bd)
{
    ...
    rb_stretched = gtk_menu_item_new_with_mnemonic (_("_Stretched"));
    ...
    g_signal_connect (rb_stretched, "activate", G_CALLBACK (set_style), bd);
...
toggle_set_background (GtkToggleButton * tb, BackdropDialog * bd)
{
    ...
    mcs_manager_set_int (bd->plugin->manager, "setbackground",
			 BACKDROP_CHANNEL, set_background);
    mcs_manager_notify (bd->plugin->manager, BACKDROP_CHANNEL);
}
